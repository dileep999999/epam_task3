...
public class HelpIndianArmy {
	...
	public static PriorityQueue<CheckPoint> buildMinHeap(Scanner sc, int n){
		...
		for(int i=0;i<n;i++){
			...
			CheckPoint c=new HelpIndianArmy.CheckPoint(parseLong(input[0])-parseLong(input[1]), parseLong(input[0])+parseLong(input[1]));
			minPQ.add(c);
		}
		return minPQ;
	}
	...
	public static void main(String args[]){
		...
		Scanner sc= new Scanner(System.in);
			...
			System.out.println(HelpIndianArmy.findMinimumDanger(minPQ, s, s, e));
			sc.close();
		}catch(Exception e){
			e.printStackTrace();
		}
